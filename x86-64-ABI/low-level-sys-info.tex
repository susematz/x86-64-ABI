...
\item[\textindex{Small code model}]
  ...
  addresses in the range from $0$ to $2^{31}-2^{24} - 1$.\footnote{
    The number $24$ is chosen arbitrarily.  It allows for all memory of
    objects of size up to $2^{24}$ to be addressed directly because the
    base address of objects is constrained to be $2^{31}-2^{24}$.
