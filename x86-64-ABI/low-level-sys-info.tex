...
Within this specification, the term \emph{\textindex{halfword}} refers
to a 16-bit object, the term \emph{\textindex{word}} refers to a
32-bit object, the term \emph{\textindex{doubleword}} refers to a
64-bit object, and the term \emph{\textindex{quadword}} refers to a
128-bit object.
...
argument will have the highest address.\footnote{Right-to-left order
  on the stack makes the handling of functions that take a variable number of
...
If the argument is a scalar type, other than \code{__long double} or
...
\reg{r9}.  \footnote{Note that \reg{r11} is neither required to
  ...
  static chain pointer.}  Arguments of floating point type other than
\code{long double}, of type
\code{__m64} and of type \code{__m128}, are placed in the next
available SSE register, taken in order from \reg{xmm0} to \reg{xmm15}.
If no registers are available, the values are placed on the stack.
...
Scalar values of type \code{long double} are always placed on the stack.

Structure or union objects with more than 16 bytes, those that contain
scalar components of type \code{long double}, or, in C++,
...
\reg{r12--r15} & callee-saved register & Yes \\
...
If a function returns a scalar of type \code{long double}, the value
is returned in \reg{st0}.  If the function returns a scalar of some
other type, the value is returned in a register or registers.  The
register or registers chosen are the same as those that would be
selected for a value of the same type to be passed to a function
taking only one argument of that same type, except that the list for
integral or pointer type scalars consists of \RAX and \RDX.
