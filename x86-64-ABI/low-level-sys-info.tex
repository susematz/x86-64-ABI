...
Aditionally arrays greater than 16~bytes are required to be aligned minimally
to the 16~byte boundary to make usage of SSE packed operations more efecient.
...
\RAX, \reg{XMM0} and \reg{MM0} are choosen according to their class.
...
The x86-64 architecture provides 64~bits of virtual space, but implementation
is allowed to support less. The processes are allowed to require only 48~bits
of address space, as supported by the initial implementation.  It is expected
the address space to monotonically grow in the future.

According the \emph{canonical address form} as defined by the x86-64
architecture, every byte of virtual space is having two addresses unless the
implementation support full 64~bit address space.  The applications may not
depend on this behaviour and address the data only using single address.

Memory management translates virtual address to physical address, hyding
physical addressing and letting a process run anywhere in the system's real
memory. Process typically begin with three logical segments called ``text'',
``data'' and ``stack''. An object file may contain more segments (for example.
for debugger use), and a process also creates additional segments for itself
with system services.

\subsection{Page Size}
Memory is organized by pages, which are the system's smallest units of
memory allocation. Page size can vary from one system to another.
Process may call \verb|sysconf|(\verb|BA_OS|) to determine current page size.
This ABI allows system to cluster memory to logical power-of-two page
sizes up to 65536 bytes.

\subsection{Virtual Address Assignments}
Conceptually process have full (minimally) 48-bit address space available.
In practice, however, several factors limit the size of a process.
\begin{itemize}
  \item The system reserves configuration dependent amount of virutal space.
...
\begin{figure}[H]
  ...
  \begin{center}
    \begin{tabular}{r|c|l}
      ...
      & \dots & \\
      \verb|0| & Process segments & Beggining of memory\\ \cline{2-2}
    \end{tabular}
  \end{center}
\end{figure}

Figure \ref{fig-address} shows the virtual address configuration on the x86-64
...
\begin{description}
  \item [Process segments]
    Processese loadable segments and stack may begin at 0. The exact addresses
    depend on the executable file format. Processes can control the amount
    of virtual memory allotted for stack space, as described below.
  \item [Dynamic segments]
    A processes dynamic segments reside below the reserved area.
...
with a processss dynamic segments below that. Although the exact boundary
between the reserved area and a process depends on the systemas configuration,
