...
This subsection discusses usage of each register.  Registers \RBP and
\reg{r10} through \reg{r15} ``belong'' to the calling function and the
...
register, taken in the order \RBX, \RDX, \RCX, \RSI, \RDI, and
\reg{r8}.  \footnote{Note that \RAX and \reg{r9} are not required to
  be preserved, nor are they used to pass arguments.  Making these
  ...
  control needs to be transferred.  \reg{r9} is used to indicate the
  number of SSE arguments passed to a function requiring a variable
  number of arguments.}  Arguments of floating point type and of type
...
bytes), doubleword of the sturcture is passed first.  If there is only
