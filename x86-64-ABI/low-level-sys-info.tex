...
\reg{r8} through \reg{r15} ``belong'' to the calling function and the
...
Arguments of floating point type and of type \code{__m128}, are placed
in the next available SSE register, taken in order from \reg{xmm0} to
\reg{xmm15}.  If no registers are available, the values are placed on
the stack.
