...
does not require all data access to be properly aligned.  Accessing
misaligned data will be slower than accessing properly aligned data,
but otherwise there is no difference.  The only exception here is that
\code{__m128} always has to be aligned properly.
...
use the faster \op{femms} instruction.}  The direction flag in the
\reg{eflags} register must be clear on function entry, and on function
return.
...
epilogue.  This area is known as \textindex{red zone}.
...
\reg{st1} & temporary registers; used to return \code{long double} arguments & No \\
...
\Hrule
  ...
  \begin{center}
    \begin{tabular}[t]{l|l|l}
      ...
      \texttt{OF} & 0 & No overflow occured\\
    \end{tabular}
  \end{center}
\Hrule
\end{table}

The direction flag \code{DF} must be set to
the ``forward'' direction (that is to zero) before entry and upon exit
from a function. Other user flags have no specified role in the
standard calling sequence and are {\em not} preserved.
...
previous material, this material is not normative.  It shows only the
difference to the \intelabi.
...
\item[\textindex{Small code model}]
  ...
  in the range from $-2^{31}$ to $2^{24}$ directly in the sign
  extended immediate operands, with offsets in the range from $0$ to
  $2^{31}-2^{24}$ in the zero extended immediate operands and use
  instruction pointer relative addressing for the symbols with offsets
  in the range $-2^{24}$ to $2^{24}$.
...
\item[\textindex{Kernel code model}]
  ...
  to be in the range from $2^{64}-2^{31}$ to $2^{64}-2^{24}$.
  ...
  offsets from $2^{31}$ to $2^{31}+2^{24}$. The range offsets for
  sign extended reference changes to $0$--$2^{31}+2^{24}$.
...
\item[\textindex{Small position independent code model} (\textindex{PIC})]
  ...
  instruction is limited to $2^{31}-2^{24}-1$, allowing the compiler
...
examples and dicussion.  They are:
...
In the small code model \xARCH does not need any function prologue for
calculating the global offset table address since it does not have an explicit
GOT pointer.
...
\begin{figure}[H]
\Hrule
\caption{Position-Independend Load and Store (Medium PIC Model)}
\begin{center}
\code{
\begin{tabular}{|l|c|ll|}
\cline{1-1}\cline{3-4}
...
\begin{figure}[H]
\Hrule
\caption{Position-Independend Load and Store (Medium PIC Model), continued}
\begin{center}
\code{
\begin{tabular}{|l|c|ll|}
\cline{1-1}\cline{3-4}
...
\begin{figure}[H]
\Hrule
\caption{Position-independend global data load and store}
...
\begin{figure}[H]
\Hrule
\caption{Faster position-independend global data load and store}
...
\subsubsection{Small and Modium Models}
...
As all labels are withing 2GB no special care has to be taken when
